# Assemble Static Site + Wasmer

This example shows how to build a **Assemble** static site and host the generated output on **Wasmer Edge**.
...
* `Gruntfile.js` runs `grunt-assemble` over templates in `src/`.
* The build writes static HTML into `dist/`.
